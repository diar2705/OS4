...
void Heap::insert(MallocMetadata *block)
{
    ...
    __num_allocated_bytes += block->__block_size;
...
void *Heap::alloc_block(size_t size)
{
    ...
    while (tmp != nullptr)
    {
        if (tmp->__is_free && tmp->__block_size >= size)
        {
            tmp->__is_free = false;
            __num_free_blocks--;
            __num_free_bytes -= tmp->__block_size;
            return tmp;
        }
        tmp = tmp->__next;
    }
...
void Heap::free_block(void *ptr)
{
    ...
    __num_free_blocks++;
    __num_free_bytes += block->__block_size;
}
