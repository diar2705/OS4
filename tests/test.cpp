...
typedef struct MallocMetadata Metadata3;
...
std::string testRealloc2(void *array[MAX_ALLOC])
{
    ...
    expected += "|F:" + std::to_string(default_block_size * 3) + "|U:" + std::to_string(default_block_size * 4) +
                "|U:" + std::to_string(default_block_size * 2) + "|";
...
std::string testSplitAndMerge(void *array[MAX_ALLOC])
{
    ...
    std::string expected = "|U:" + std::to_string(small_part_of_block) +
                           "|F:" + std::to_string(default_block_size - small_part_of_block - size_of_metadata);
    expected += "|U:" + default_block;
    expected += "|U:" + default_block;
 ...
 *
 * NOTE - spring 2022, this test was edited according to a piazza answer
 * that there should be NO 2 consecutive free blocks in the free list
 * i.e we should ALWAYS merge if possible
 * @param array
 * @return
 */
std::string testNoRecMerge(void *array[MAX_ALLOC])
{
    ...
    start += "|F:" + std::to_string(2 * default_block_size - (default_block_size / 3));
    ...
    printMemory<Metadata3>(memory_start_addr, true);
    expected += "|U:" + std::to_string(default_block_size / 3);
    expected += "|F:" + std::to_string(default_block_size * 3 - (default_block_size / 3) + size_of_metadata);
    expected += "|";
...
TestFunc functions[NUM_FUNC] = {testInit,
                                allocNoFree,
                                allocandFree,
                                testFreeAllAndMerge,
                                allocandFreeMerge,
                                ...
                                testReallocMMap,
                                testReallocDec,
                                testReallocDecOverwrite,
                                testNoRecMerge,
                                testReallocWildLikePiazza,
                                testReallocWilderness2,
                                mergeVariations};
std::string function_names[NUM_FUNC] = {"testInit",
                                        "allocNoFree",
                                        "allocandFree",
                                        "testFreeAllAndMerge",
                                        "allocandFreeMerge",
...
void printInitFail()
{
    ...
    std::cerr << "The test get the start of the memory list using an allocation of size 1 and free it right after"
              << std::endl;
...
void printDebugInfo()
{
    ...
    std::cout << "Default 3 block after merge size is: " << default_block_size * 3 + size_of_metadata * 2 << std::endl
              << std::endl;
}
